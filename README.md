...
> README репозитория `unique-visitors-badge`. ОПУБЛИКОВАН 2026-08-15.
>
> 🎯 Кликбейт-заголовок (согласован с пользователем 2026-08-15): **«Твой GitHub-профиль считает ПРИЗРАКОВ. Узнай, сколько РЕАЛЬНЫХ людей тебя посещают»**.
>
> 🔄 **Обновление 2026-08-15:** добавлена секция «Ограничения и частые вопросы (FAQ)» — про владельца, задержки Traffic API, кэши и тестирование. ✅ **Опубликовано в GitHub.** Дополнено: «Можно ли сделать период больше 14 дней» — накопленная история позволяет строить месячные/годовые периоды. ✅ **Опубликовано в GitHub.**
>
> 📊 **Обновление 2026-08-15 (аналитика):** добавлен `scripts/analytics.py` — лаконичная аналитика из накопленной истории (тренд, max uniques в окне, сумма count за 7/30 дней/всё время, `analytics.json`). FAQ уточнён: `sum_count` — это верхняя граница (окна пересекаются), а не точная сумма просмотров. ✅ **Опубликовано в GitHub** (analytics.py, analytics.json, workflow, README; workflow success, analytics.json сгенерирован).
>
> ⏳ **Обновление 2026-08-15 (FAQ для форкающих):** в начало FAQ добавлен явный блок «Если ты форкнул этот репозиторий» — про тройную задержку обновления (Traffic API 1–2 ч + срез раз в сутки 06:00 UTC + кэш бейджа до 24 ч). **Готово к публикации (ждёт подтверждения).**
...
>
> 1. **GitHub Traffic API обновляется с лагом 1–2 часа** (иногда дольше) — заход появляется в API не сразу.
> 2. **Workflow снимает срез раз в сутки** — в **06:00 UTC (09:00 МСК)**. Между срезами бейдж показывает прошлое значение.
> 3. **Бейдж кэшируется до 24 часов** (`cacheSeconds: 86400`) — shields.io и GitHub Camo держат старую картинку.
>
> Итого: от захода до новой цифры на бейдже может пройти **до 2 суток**. Это нормально и не значит, что счётчик сломан.
> Проверять актуальные данные лучше по `badge.json` / `analytics.json` в репозитории — они обновляются раньше, чем картинка.
